...
// 为OKButton注册一个内部监听类
        OKButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                for (int i = 0; i < access.length; i++) {
                    ...
                    if (access[i][0] == null && access[i][1] == null && access[i][2] == null && access[i][3] == null && access[i][4] == null) {
                        count = i;
                        System.out.println(count);
                        // System.out.println(i);
                        break;
                    }
                    if (access[i][0].equals("") && access[i][1].equals("") && access[i][2].equals("") && access[i][3].equals("") && access[i][4].equals("")) {
                        count = i;
                        System.out.println(count);
                        // System.out.println(i);
                        break;
                ...
                if (getSeek(text[0]) == Integer.MIN_VALUE && getSeek(text[1]) == Integer.MIN_VALUE && getSeek(text[2]) == Integer.MIN_VALUE) {
    ...
    System.out.println(access[count][i]);
                        ...
                        int n = JOptionPane.showConfirmDialog(null, "添加成功！！", "提示", JOptionPane.DEFAULT_OPTION);
...
// System.out.println("查找内容不存在");
    ...
    public void makeAlter() {
        int n = JOptionPane.showConfirmDialog(null, "确定修改吗？", "提示：", JOptionPane.OK_CANCEL_OPTION);
        if (n == JOptionPane.YES_OPTION) {
            for (int i = 0; i < access[index].length; i++) {
